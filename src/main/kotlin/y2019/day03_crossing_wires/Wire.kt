...
class Wire {
    ...
    val path : MutableSet<ScreenCoordinate> = HashSet()
    val stepsToTracker : MutableMap<ScreenCoordinate, Int> = HashMap()

    fun tracePath(path: String) {

        path.split(',')
                .map { toSection(it) }
                .forEach { traceSection(it) }


    }

    private fun traceSection(section: WireSection) {
        for (i in 0 until section.length) {
            val nextLocation = currentLocation.next(section.direction)
            steps++
            stepsToTracker.putIfAbsent(nextLocation, steps)
            path.add(nextLocation)
            currentLocation = nextLocation
        }
    }

    private fun toSection(section: String): WireSection {
        val direction = parseDirection(section[0])
        val length = section.substring(1).toInt()
        return WireSection(direction, length)
    }

    fun crossPathsWith(other: Wire): Set<ScreenCoordinate> {
        return this.path.intersect(other.path)
    }

    fun stepsTo(intersection: ScreenCoordinate): Int {
        return stepsToTracker[intersection]!!
    }


}

data class WireSection(val direction: Direction, val length: Int)

fun wireOf(path: String): Wire {
    val wire = Wire()
    wire.tracePath(path)
    return wire
}

fun findDistanceToNearestIntersection(lines: List<String>): Int {
    return findDistanceToNearestIntersection(wireOf(lines[0]), wireOf(lines[1]))
}

private fun findDistanceToNearestIntersection(first: Wire, second: Wire): Int {
    val crossPathsWith = first.crossPathsWith(second)
    val nearestIntersection = crossPathsWith
            .sortedBy { it.distanceTo(ORIGIN) }
            .first()

    return nearestIntersection.distanceTo(ORIGIN)
}


fun findDistanceToFirstIntersection(lines: List<String>): Int {
    return findDistanceToFirstIntersection(wireOf(lines[0]), wireOf(lines[1]))
}

fun findDistanceToFirstIntersection(first: Wire, second: Wire): Int {
    val intersections = first.crossPathsWith(second)

    return intersections
            .map { first.stepsTo(it) + second.stepsTo(it) }
            .sorted()
            .first()
}
