...
class LinearAmplificationCircuitTest {
    ...
    fun exampleOne() {
        ...
        assertThat(findMaxThrustSignalInLinearConfiguration(compile(program)).intValueExact(), equalTo(43210))
    }
    ...
    fun simulateOnePermutation() {
        ...
        assertThat(thrustSignal.intValueExact(), equalTo(42310))
    }

    @Test
    fun exampleTwo() {
        val program = "3,23,3,24,1002,24,10,24,1002,23,-1,23,101,5,23,23,1,24,23,23,4,23,99,0,0"
        assertThat(findMaxThrustSignalInLinearConfiguration(compile(program)).intValueExact(), equalTo(54321))
    }

    @Test
    fun exampleThree() {
        val program = "3,31,3,32,1002,32,10,32,1001,31,-2,31,1007,31,0,33,1002,33,7,33,1,33,31,31,1,32,31,31,4,31,99,0,0,0"
        assertThat(findMaxThrustSignalInLinearConfiguration(compile(program)).intValueExact(), equalTo(65210))
    }

    @Test
    fun verifyPartOne() {
        val program = AMPLIFIER_CONTROLLER_SOFTWARE
        assertThat(findMaxThrustSignalInLinearConfiguration(compile(program)).intValueExact(), equalTo(70597))
