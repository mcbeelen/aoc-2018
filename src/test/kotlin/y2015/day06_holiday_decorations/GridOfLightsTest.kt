package y2015.day06_holiday_decorations
 ...
 */
class GridOfLightsTest {

    @Test
    fun `all lights should start off`() {
        val grid = GridOfLights()

        assertThat(grid.countNumberOfLightsLit(), equalTo(0))
    }

    @Test
    fun name() {
        val grid = GridOfLights()
        grid.processInstruction("turn on 0,0 through 2,2")
        assertThat(grid.countNumberOfLightsLit(), equalTo(9))
        grid.processInstruction("toggle 0,0 through 999,999")
        assertThat(grid.countNumberOfLightsLit(), equalTo(999_991))
        grid.processInstruction("turn off 499,499 through 500,500")
        assertThat(grid.countNumberOfLightsLit(), equalTo(999_987))
    }
